...
func writeToFile(filepath string, context string) {
	...
	w.WriteString(context) // 쓰기 인스턴스로 버퍼에 Hello, world! 쓰기
	w.Flush()
...
func readFile(filepath string) {
	...
	file.Seek(0, io.SeekStart) // 파일 읽기 위치를 파일의 맨 처음(0)으로 이동
